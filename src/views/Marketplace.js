...
export default function Marketplace() {
  ...
  const _tabIndex =
    localStorage.getItem('marketplace_tab_index') === null
      ? 0
      : localStorage.getItem('marketplace_tab_index') * 1;
  ...
  const getListedNFTs = useCallback(async () => {
    ...
    try {
      ...
      const listedOrdersArr = [];
      for (let i = 0; i < ordersArr.length; i++) {
        const _order = Object.assign(
          [],
          await NFTMarketplaceConnectedContract.orders(ordersArr[i])
        );
  ...
  const getListedNFTsFromDB = useCallback(async () => {
    ...
    try {
      const _totalOnsale = await getOnsaleCount(
      ...
      if (response.status !== 200) {
        // console.log(
        //   '#102 ERROR while loading data from database, try to load from blockchain...'
        // );;
        getListedNFTs();
      }
      ...
      for (let i = 0; i < response.data.data.length; i++) {
        ...
        listedOrdersArr.push({
          ...
          _artifacts:
            data.artifacts === null
              ? null
              : ethers.BigNumber.from(data.artifacts),
          _dna: data.dna,
          _hexArtifacts:
            data.artifacts === null
              ? null
              : ethers.BigNumber.from(data.artifacts).toHexString(),
        });
      }
      setListedNFTs(listedOrdersArr);
      setIsLoading(false);
      ...
      // console.log(
      //   '#101 ERROR while loading data from database, try to load from blockchain...'
      // );;
      getListedNFTs();
    }
  }, [
  ...
  const itemList = (type) => {
    return type === 0 ? <OnSaleNFTs /> : <MyListedNFT />;
  };
      ...
      <Box m="auto" w={['40%', null, '60%']} mb="10">
        <Pagination
          ...
          pageCount={
            parseInt(
              ((tabIndex === 0
                ? totalOnsale.onsaleCount
                : totalOnsale.myListedCount) -
                1) /
                pageSize
            ) + 1
          }
          totalRecords={
            tabIndex === 0 ? totalOnsale.onsaleCount : totalOnsale.myListedCount
          }
