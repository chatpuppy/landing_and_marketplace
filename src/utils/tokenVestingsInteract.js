...
export const loadTotalParticipant = async (participant) => {
  const response = await tokenVestingContract.getTotalAmountByParticipant(
    participant
  );
  return response;
};
...
export const loadBeneficiaryCount = async (participant) => {
  const response = await tokenVestingContract.getBeneficiaryCountParticipant(
    participant
  );
  return response;
};
 ...
 * @returns
 */
export const loadBeneficiary = async (index) =>
  await tokenVestingContract.getBeneficiary(index);

/**
 * Get all beneficiary count
 */
export const loadBeneficiaryCountAll = async () =>
  await tokenVestingContract.getBeneficiary();
