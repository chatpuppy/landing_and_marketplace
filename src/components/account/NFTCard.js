...
const NFTCard = (props) => {
  ...
  const [unboxModalParams, setUnboxModalParams] = useState({
    tokenId: 0,
    artifacts: 0,
    dna: '',
  });
  ...
  const onClose1 = () => {
    setIsOpen1(false);
    handleUnboxDone(number, unboxModalType);
  };
  ...
  const strLevel =
    'LEVEL: <br/>Sum of levels of each traits, <br/>higher means more value.';
  const strExperience =
    "EXPERIENCE: <br/>Sum of each trait's experience, <br/>higher means more value.";
  const strRarity =
    'RARITY: <br/>Probability of same NFT<br/> in 1,000,000 NFTs, <br/>lower means more value.';
  const strArtifacts = 'ARTIFACTS: <br/>Artifacts of NFT on chain';
  const strMeta =
    "Metadata on IPFS <br/>If you did't upload while unboxing, <br/>you can upload to IPFS now";
  ...
  const unboxNFT = async () => {
    ...
    if (
      ...
      NFT_core_contract_address === undefined
    ) {
      toast({
        title: 'Unbox NFT',
        ...
        try {
          ...
          try {
            ...
            if (currentNetwork === 80001)
              tx = await NFTManagerConnectedContract.unbox(number);
            ...
            setConfirmationProgressData({
              step: '3/4',
              value: 75,
              message: `Generating random NFT metadata by ChainLink, it will take around ${needSeconds}"`,
            });
            ...
            const t = setInterval(async () => {
              ...
              const _metadata = await NFTCoreConnectedContract.tokenMetaData(
                number
              );
              if (_metadata._artifacts > 0) {
                ...
                setConfirmationProgressData({
                  step: '4/4',
                  value: 100,
                  message: 'Congrat! you have got an exclusive NFT',
                });
                clearInterval(t);
  ...
  const getUri = (_uri) => {
    ...
    else if (_uri.substr(0, 2) === 'Qm')
      return 'ipfs://' + _uri + '/metadata.json';
    else if (_uri.substr(0, 7) === 'bafyrei')
      return 'ipfs://' + _uri + '/metadata.json';
  };
  ...
  useEffect(() => {
    ...
    if (needSeconds === 0) {
      const desc =
        "Unboxing is out of time, don't worry, you can open your list a few minutes later.";
      toast({
        title: 'Unbox NFT',
        description: desc,
        status: 'warning',
        ...
        <Image
          ...
          src={
            unboxed && parsedMetadata !== null
              ? imageBase64 === ''
                ? `./images/loading${getBackgroundId(metadata)}.jpg`
                : imageBase64
              : src
          }
          alt="NFT Avatar"
            ...
            {unboxed && parsedMetadata !== null ? (
              <Flex>
                ...
                {tokenUri === '' ? (
                  ...
                  <a href={getUri(tokenUri)} target="_blank" rel="noreferrer">
                    &nbsp;&nbsp;Open in IPFS
                  </a>
                )}
              </Flex>
        ...
        {unboxed ? (
          ...
          <Box>
            <Text ml={5} mr={5} mt={5} color={'gray.500'}>
              Click UNBOX button to open this mystery box, or click SELL button
              to list it on marketplace.
            </Text>
            <Flex
              alignItems="center"
              ...
              networkConfig.buttons.unbox.visible ? (
                <Button
                  size="md"
                  bg={buttonbg}
                  color={bg}
