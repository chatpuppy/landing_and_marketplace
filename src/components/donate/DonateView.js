...
export const DonateView = () => {
  ...
  const format = (num) => {
    return formatThousands(parseFloat(num).toFixed(0).toString(), {
      separator: ',',
    });
  };
  ...
  useEffect(() => {
    ...
    setProgressRate({
      ...
      rate:
        (participantTotal / priceRange[priceRange.length - 1].fromAmount) * 100,
    });
  ...
  return (
    <Stack
      ...
      {countdownOptions.timeTillDate === 0 ? (
        ...
        <Card textAlign={'center'} justifyContent={'center'} w={'full'} p={5}>
          <Countdown
      ...
      <Card textAlign={'center'} justifyContent={'center'} w={'full'} p={5}>
        ...
        <Progress
          ...
          colorScheme="green"
        />
        <Box letterSpacing={'2px'} fontSize={'md'} mt={5}>
          {format(ethers.utils.formatEther(progressRate.sold)) +
            ' CPT / ' +
            format(ethers.utils.formatEther(progressRate.total)) +
            ' CPT = ' +
            progressRate.rate.toFixed(4) +
            '%'}
        </Box>
        <Box
          letterSpacing={'2px'}
          fontSize={'md'}
          mt={5}
          textTransform={'uppercase'}>{`Current Rate: ${format(
          donationPrice.price
        )} CPT/BNB`}</Box>
      </Card>
      ...
      <Stack
        ...
        <SimpleGrid
          ...
          <Card textAlign={'center'} justifyContent={'center'}>
            ...
            {beneficiaryData === undefined ? (
              ...
              <Text
                ...
                {'Donated ' +
                  ethers.utils.formatEther(
                    beneficiaryData.totalAmount.div(beneficiaryData.price)
                  ) +
                  ' BNB x ' +
                  format(beneficiaryData.price) +
                  ' CPT/BNB'}
              </Text>
            )}
            <Text
              ...
              {beneficiaryData === undefined
                ...
                : 'at ' +
                  DateTime.fromSeconds(
                    parseInt(beneficiaryData.timestamp)
                  ).toFormat('F')}
            </Text>
            {beneficiaryData === undefined ||
            beneficiaryData.totalAmount.eq(beneficiaryData.releasedAmount) ||
            !showRedeemButton ? (
              <></>
            ) : (
              <Button
          ...
          <Card textAlign={'center'} justifyContent={'center'}>
            ...
            <Text mt={-3} ml={8}>
              {beneficiaryData === undefined ? (
                <AiTwotoneCheckCircle color={'gray'} />
              ) : beneficiaryData.status === 1 ? (
                <AiTwotoneCheckCircle color={'#48BB78'} />
              ) : (
                <AiTwotoneCheckCircle color={'#E53E3E'} />
              )}
            </Text>
            ...
            <Text mt={2} fontSize={'3xl'}>
              {format(
                ethers.utils.formatEther(
                  beneficiaryData === undefined
                    ? 0
                    : beneficiaryData.releasedAmount
                )
              )}
            </Text>
            <Heading
            ...
            <Text mt={0} mb={2} fontSize={'3xl'}>
              {format(
                ethers.utils.formatEther(
                  releasable === undefined ? 0 : releasable
                )
              )}
            </Text>
            ...
            {releasable > 0 && showReleaseButton ? (
              <Button
                ...
                onClick={() => {
                  setModalOptions({
                    message: `Do you want to claim ${format(
                      ethers.utils.formatEther(
                        releasable === undefined ? 0 : releasable
                      )
                    )} CPT?`,
      ...
      {participantID === 1 || participantID === 2 ? (
        <Box>
          ...
          ) : new Date().getTime() < donateData.startTimestamp * 1000 ? (
            <Card>
              <DonotedBox message={'Donation has not started!'} />
            </Card>
          ) : beneficiaryData !== undefined && beneficiaryData.timestamp > 0 ? (
            <Card>
...
const DonotedBox = (props) => {
  return (
    <Box
      ...
      mr={20}
      mb={10}>
      <Box fontSize={'3xl'} fontWeight={'bold'} mb={10} color={'#E53E3E'}>
        {props.message}
      </Box>
    </Box>
  );
  ...
  return (
    <>
      <Box
        ...
        // p={5}
      >
        <HStack maxW="full" color={useColorModeValue('black', '#dcdcdc')}>
          <Button {...dec}>-</Button>
          <Input {...input} />
          <Button {...inc}>+</Button>
        </HStack>
          ...
          {priceRange.length > 0 &&
            priceRange.map((range, idx) => (
              <Tr key={idx}>
                <Td>{idx + 1}</Td>
                <Td>
                  {formatThousands(ethers.utils.formatEther(range.fromAmount), {
                    separator: ',',
                  })}{' '}
                  CPT
                </Td>
                <Td>
                  {idx === priceRange.length - 1
                    ? 'CAP'
                    : formatThousands(range.price.toString(), {
                        separator: ',',
                      }) + ' CPT/BNB'}
                </Td>
              </Tr>
